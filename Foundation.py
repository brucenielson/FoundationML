...
def stats(clf, X, y):
    ...
    print_stats("Positive Predictions", X_postitive, y_positive, y_positive_pred)
...
def test(clf, X, y):
    # returns statistics
    y_pred = clf.predict(X)
    y_pred = y_pred[0:len(X)]
    print("")
    print('Total Samples:', len(X))
    print("Results of Predict - Test Set:")
    print('Misclassified Test samples: %d' % (y != y_pred).sum())
    print('Accuracy of Test set: %.4f' % accuracy_score(y, y_pred))
